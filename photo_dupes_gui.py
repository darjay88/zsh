...
from tkinter import Tk, Button, Label, Entry, Frame, Listbox, Scrollbar, Toplevel, Checkbutton, StringVar, messagebox, Canvas, Scrollbar as ScrollbarV, HORIZONTAL, VERTICAL
    ...
    def __init__(self, parent, group):
        ...
        for p in group:
            try:
                ...
                self.canvas.create_text(x, y + thumb_size + 5, text=str(p).split(os.sep)[-1], anchor='nw', font=('Arial', 8))
                x += thumb_size + 20
                if x > 700:
                    x = 10
                    y += thumb_size + 40
...
class PhotoDuplicateGUI:
    def __init__(self):
        self.root = Tk()
        self.root.title('Photo Duplicate Tool - Thumbnails')
        ...
        Label(self.threshold_frame, text='Threshold:').pack(side='left')
        ...
        self.listbox.bind('<Double-1>', self.preview_thumbnails)
    ...
    def find_duplicates(self):
        ...
        try:
            ...
            if mode == 'exact':
                self.groups = exact_duplicates(folder)
            else:
                self.groups = similar_photos(folder, threshold)
    ...
    def preview_thumbnails(self, event):
        selection = self.listbox.curselection()
        if not selection:
            return
        group_idx = selection[0] // (len(self.groups[0]) + 2) if self.groups else 0
        if group_idx < len(self.groups):
            ThumbnailViewer(self.root, self.groups[group_idx])
